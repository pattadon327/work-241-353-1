...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            accuracy = results[0]  # Assuming Precision as Accuracy
            writer.add_scalar('Metrics/Accuracy', accuracy, epoch)

            # Log other metrics if needed
            writer.add_scalar('Metrics/mAP@0.5', results[2], epoch)  # mAP@0.5

            # Log additional metrics to TensorBoard
            writer.add_scalar('Loss/Box Regression', mloss[0], epoch)
            writer.add_scalar('Loss/Objectness', mloss[1], epoch)
            writer.add_scalar('Loss/Box Classification', mloss[2], epoch)
            writer.add_scalar('Loss/Segmentation', mloss[3], epoch)
            writer.add_scalar('Learning Rate', lr[0], epoch)

            # Log Precision-Recall Curve
            writer.add_pr_curve('Precision-Recall', results[4], results[5], epoch)  # Assuming results[4] and results[5] are precision and recall arrays
